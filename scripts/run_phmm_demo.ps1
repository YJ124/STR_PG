...
cmd /d /c "python -m strpg.cli genotype --registry demo_output/registry.jsonl --gaf demo_output/demo.gaf --fq1 examples/demo_R1.fastq --fq2 examples/demo_R2.fastq --sample-id demo --read-cache-dir demo_output/phmm_read_cache --likelihood-backend phmm --out demo_output/genotypes.phmm.tsv 2> demo_output/genotype.phmm.stats.json"
if ($LASTEXITCODE -ne 0) { throw "pHMM genotype demo failed with exit code $LASTEXITCODE" }
